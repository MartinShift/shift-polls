...
<div class="container mx-auto" id="app">
    <section class="p-10 px-0">
        <ol class="list-none p-0 inline-flex">
            ...
            <li>
                <a href="#" class="text-gray-500" aria-current="page">{{__(('Edit'))}}</a>
            </li>
        </ol>
    </section>
    <div class="w-full">
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="mb-6">
                ...
                <input v-model="question" placeholder="{{__(('Question'))}}" class="shadow appearance-none border rounded w-full py-4 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="password" type="text">
            </div>
            <div class="mb-6">
                <label for="question-logo" class="block text-gray-700 text-lg font-bold mb-2 uppercase tracking-wide font-bold">
                {{__(('QuestionLogo'))}}
                </label>
                <input @change="onFileChange" placeholder="{{__(('QuestionLogo'))}}"
                    ...
                    id="question-logo" type="file">
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 text-lg font-bold mb-2 uppercase tracking-wide font-bold" for="question">
                {{__(('Description'))}}
                </label>
                <textarea v-model="description" placeholder="{{__(('Description'))}}" class="shadow appearance-none border rounded w-full py-4 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="description">
                </textarea>
            </div>
            <div class="flex flex-wrap mb-6">
                ...
                <div class="w-full flex items-center border-b border-b-2 border-teal-500 py-2">
                    <input @keyup.enter="addNewOption" v-model="newOption" class="appearance-none bg-transparent border-none block w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" placeholder="{{__(('OptionText'))}}" aria-label="Full name">
            ...
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    ...
                    <input  id="start_at" v-model="startDate"  class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white localDates" type="text">
                    <p class="text-red-500 text-xs italic hidden">{{__(('PleaseFill'))}}</p>
                </div>
                <div class="w-full md:w-1/2 px-3">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
                    {{__(('EndsAt'))}}
                    </label>
                    <input id="end_at" v-model="endDate" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 localDates" type="text">
                </div>
            </div>
    ...
    <script>
        ...
        new Vue({
           el: '#app',
            ...
            data() {
                var model = {!! json_encode($question)!!};
                console.log(model);
            return {
                newOption: '',
                ...
                model: model,
                startDate : model.start_at,
                endDate : model.end_at,
               ...
               addNewOption(){
                ...
                save(){
                   ...
                   if(document.getElementById('end_at').value !== ''){
                       data.end_at = document.getElementById('end_at').value;
                   }
                ...
                formData.append(`options[${index}]`, option.value);
                    ...
                    axios.post("{{route('questions.update')}}", formData, {
                        headers: {
